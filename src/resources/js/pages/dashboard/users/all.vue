<template>
  <card v-if="users_meta">
    ...
    <template v-slot:header>
      <router-link :to="{name: 'dashboard.user.add'}" class="btn btn-outline-primary">
        <fa icon="plus"/>
      </router-link>
    </template>
    ...
    <table v-if="users && users.length" class="table table-bordered table-hover">
      ...
      <tr v-for="(user, key) in users" :key="key">
        ...
        <td class="text-center">
          ...
          <fa icon="trash" class="text-danger" @click="destroy(user)" role="button"/>
        </td>
      </tr>
      </tbody>
    </table>
...
export default {
  ...
  methods: {
    edit(data) {
      ...
      router.push({name: 'dashboard.user.edit', params: {id: data.id}})
    },
    destroy({id, company_id}) {
      const page = this.getPage()
      store.dispatch('deleteCompanyUser', {id, company_id, page})
    },
  ...
  computed: {
    ...mapGetters({
      users: 'company_users',
      users_meta: 'company_users_meta',
    })
  },

  async beforeRouteEnter(to, from, next) {
    await store.dispatch('fetchCompanyUsers', to.query.page)
    next()
  },
